...
// The same PSViewController + UIKit child arrangement used by GlowIconPosition.
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section { return section == 0 ? 5 : 1; }
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    return @[@"First notification position", @"Apply changes", @"About"][section];
}
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section {
    if (section == 0) return @"Choose where the first notification starts. Negative values move up; positive values move down. 0 keeps the original position. Notifications and groups grow downward from here.";
    if (section == 1) return @"Respring after changing the position to apply it.";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)path {
    ...
    if (path.section == 0 && path.row < 2) {
        ...
        if (path.section == 0) cell.textLabel.text = @[@"Move up 5 pt", @"Move down 5 pt", @"Reset to default position"][path.row - 2];
        else if (path.section == 1) cell.textLabel.text = @"Respring";
        else {
